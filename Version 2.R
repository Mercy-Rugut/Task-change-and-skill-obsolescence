...
keep_vars <- ses_data[, c("dataset", "asex", "aage","bauto", "bempsta", "bemptype", 
                          ...
                          "dquals1","dquals2", "dpaidwk", "esector", "jchange","kethnic",
                          ...
                          "cwritelg","ccalca","cpercent","cstats","ccoop",          
...
selected_data1$qualification[
  selected_data1$dquals1 %in% c(23, 24) &
...
knowledge_based <- c("ccause","csolutn","canalyse","clong","cwritelg","cstats",
                      "cspecial","corgwork","cproduct","ctools","cforlan")

experience_based <- c("cdetail","cpeople","cteach","cspeech","cpersuad","cselling",
                       "ccaring","cteamwk","clisten","cstrengt","cstamina","chands",
                       "cfaults","cnoerror","cmistake","cplanme","cplanoth","cmytime",        
                       "cahead","cread","cshort","cwrite","cwritesh","ccalca","cpercent",
                       "ccoop","cmotivat","cthings","ccoach","ccareers","cfuture",
                       "cmefeel","cothfee","clookpr","csoundp")

# Weighted task group
knowledge_w <- paste0(knowledge_based, "_w")
experience_w <- paste0(experience_based, "_w")
task_vars_w<- paste0(task_vars, "_w")

# Share of weighted task measure
# Knowledge based
selected_data1$knowledge_share <- with(selected_data1,
                            rowSums(selected_data1[knowledge_w], na.rm = TRUE) /
                            rowSums(selected_data1[task_vars_w], na.rm = TRUE) * 100)

# Experience based
selected_data1$experience_share <- with(selected_data1,
                                       rowSums(selected_data1[experience_w], na.rm = TRUE) /
                                         rowSums(selected_data1[task_vars_w], na.rm = TRUE) * 100)
...
write.csv(new_vars, "ses_data.csv", row.names = FALSE)
